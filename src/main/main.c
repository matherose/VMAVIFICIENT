...
int main(int argc, char *argv[]) {
  ...
  if (!ctx->opt.scale_to_hd) {
    ...
    ui_kv("Output", "%s%s", ctx->output_dir, ctx->output_name);
...

  {
    ...
    char rpu_path[4096] = "";
    if (!ctx->opt.scale_to_hd && !ctx->opt.dry_run && !ctx->opt.grain_only) { /* 4K encode block */
      if (ctx->hdr.error == 0 && ctx->hdr.has_dolby_vision) {
        char rpu_name[2048];
        build_rpu_filename(rpu_name, sizeof(rpu_name), ctx->base_name);
      ...
      snprintf(av1_video_name, sizeof(av1_video_name), "%s.video.mkv", ctx->base_name);
      ...
      /* ---- Final MKV muxing ---- */
      {
        ...
        snprintf(final_path, sizeof(final_path), "%s%s", ctx->output_dir, ctx->output_name);
        ...
        if (audio_fail_count > 0) {
          ...
          ui_stage_fail(ctx->output_name, "video encode failed — mux skipped");
          pipeline_failed = 1;
        } else {
          mr = final_mux(&mux_cfg);

          if (mr.skipped) {
            ui_stage_skip(ctx->output_name, "already exists");
          } else if (mr.error == 0) {
            char detail[64];
            snprintf(detail, sizeof(detail), "%s", ui_fmt_duration(difftime(time(NULL), mux_t0)));
            ui_stage_ok(ctx->output_name, detail);
          } else {
            char err[128];
